...
4. Reload the worker nodes in your cluster for the changes to take effect.
...
worker                         Ready                      worker   5h48m   v1.20.0
worker2                        Ready                      worker   5h48m   v1.20.0
```

5. Create the IBM Operator catalog source.
```bash
cat <<EOF |oc apply -f -
...
6. Create the following catalog source to ensure that dependencies can be installed:
```bash
cat <<EOF |oc apply -f -
---
apiVersion: operators.coreos.com/v1alpha1
